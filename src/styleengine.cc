   ...
   for (int j = 0; j < props->size (); j++) {
      ...
      switch (p->name) {
         ...
         case CSS_PROPERTY_MARGIN_BOTTOM:
            ...
            if (isAbsLength(attrs->margin.bottom) && absLengthVal(attrs->margin.bottom) < 0) // \todo fix negative margins in dw/*
               attrs->margin.bottom = createAbsLength(0);
            break;
         case CSS_PROPERTY_MARGIN_LEFT:
            computeLength (&attrs->margin.left, p->value.lenVal, attrs->font);
            if (isAbsLength(attrs->margin.left) && absLengthVal(attrs->margin.left) < 0) // \todo fix negative margins in dw/*
               attrs->margin.left = createAbsLength(0);
            break;
         case CSS_PROPERTY_MARGIN_RIGHT:
            computeLength (&attrs->margin.right, p->value.lenVal, attrs->font);
            if (isAbsLength(attrs->margin.right) && absLengthVal(attrs->margin.right) < 0) // \todo fix negative margins in dw/*
               attrs->margin.right = createAbsLength(0);
            break;
         case CSS_PROPERTY_MARGIN_TOP:
            computeLength (&attrs->margin.top, p->value.lenVal, attrs->font);
            if (isAbsLength(attrs->margin.top) && absLengthVal(attrs->margin.top) < 0) // \todo fix negative margins in dw/*
